...
// Endpoint to add a new version
app.post("/versions/:projectId", async (req, res) => {
  try {
    const collection = await connectToVersionConnection();
...
app.post("/editVersion/:id", async (req, res) => {
  ...
  try {
    ...
    const { serialNo, versionName, description, startDate, endDate } = req.body;
    const result = await collection.updateOne(
      { _id: new ObjectId(versioneditId) },
      { $set: { serialNo, versionName, description, startDate, endDate } }
    );
    if (result.modifiedCount === 1) {
      res.status(200).json({ message: "Project edited successfully" });
    } else {
...
app.get(
  "/testcases/byprojectandversion/:projectName/:versionName",
  async (req, res) => {
    const { projectName, versionName } = req.params;
    try {
      const collection = await connectToTestcaseConnection();
      const testcases = await collection
        .find({ projectName, versionName })
        .toArray();
...
app.get("/testcases/names", async (req, res) => {
  try {
    const collection = await connectToTestcaseConnection();
    const testcases = await collection.find().toArray();
    const testcasesNames = Array.from(
      new Set(testcases.map((testcase) => testcase.title))
    );
...
app.post("/editTestcase/:id", async (req, res) => {
  ...
  try {
    const collection = await connectToTestcaseConnection();
    const { title, input2, input3, input4, input5, input7 } = req.body; // Correctly destructure the testcase data
    console.log(req.body);
    const result = await collection.updateOne(
      { _id: new ObjectId(testcaseEditId) },
      { $set: { title, input2, input3, input4, input5, input7 } }
    );
    console.log("Result:", result);
    if (result.modifiedCount === 1) {
      res.status(200).json({ message: "Testcase edited successfully" }); // Correct the success message
    } else {
      res.status(404).json({ error: "Testcase not found" }); // Correct the error message
    }
  } catch (error) {
